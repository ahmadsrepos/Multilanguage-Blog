        ...
        <div class="row">
            ...
            <div class="row">
                <div class="form-group col-sm-12">
                    <a href="{{route('categories.create')}}" class="btn btn-primary">{{__('words.create_new_category')}}</a>
                </div>
            </div>
            @endcan
        @if ($errors->any())
                        ...
                        <table class="table datatableActivate table-responsive" style="overflow-x: auto">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">{{__('words.parent_category')}}</th>
...
<script>
    ...
    {
        $('#deleteForm').attr('action', '/dashboard/categories/'+$(this).data('id'));
    });
    $(document).on('click', '.restore-btn', function()
    {
        $('#restoreForm').attr('action', '/dashboard/categories/restore/'+$(this).data('id'));
    });
    $(document).on('click', '.finalDelete-btn', function()
    {
        $('#finalDeleteForm').attr('action', '/dashboard/categories/final-delete/'+$(this).data('id'));
    });
</script>
@endpush