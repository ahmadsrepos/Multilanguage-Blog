        ...
        <div class="row">
            ...
            <div class="row">
                <div class="form-group col-sm-12">
                    <a href="{{route('posts.create')}}" class="btn btn-primary">{{__('words.create_new_post')}}</a>
                </div>
            </div>
        @if ($errors->any())
...
<script>
    ...
    {
        $('#deleteForm').attr('action', '/dashboard/posts/'+$(this).data('id'));
    });
    $(document).on('click', '.restore-btn', function()
    {
        $('#restoreForm').attr('action', '/dashboard/posts/restore/'+$(this).data('id'));
    });
    $(document).on('click', '.finalDelete-btn', function()
    {
        $('#finalDeleteForm').attr('action', '/dashboard/posts/final-delete/'+$(this).data('id'));
    });
</script>
@endpush